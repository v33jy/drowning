...
- 관제 앱과 요구조자 앱은 마이크가 계속 열린 일반 양방향 통화를 사용합니다.
- 요구조자 앱에서는 통화 중 마이크를 음소거하거나 다시 켤 수 있습니다.
- 일시적인 연결 실패는 2초 간격으로 최대 3회 자동 재연결하고, 이후에는 수동 재시도를 표시합니다.
- 마이크 권한이 꺼져 있으면 기기 설정으로 이동하도록 안내하며, 권한을 허용한 뒤 통화를 다시 연결할 수 있습니다.
- 관제 앱은 통화 중 마이크 입력 감지 여부와 입력 레벨을 표시합니다.
...
아래 절차는 macOS 개발 PC에서 서버와 구조자용 관제 앱을 실행하고, Android `Medium Phone`
에뮬레이터에서 요구조자 앱을 실행하는 기준입니다. 세 터미널을 각각 사용합니다. 스피커의 소리가
다시 마이크로 들어가면 하울링이 생기므로 관제 PC에는 이어폰 또는 헤드셋을 연결하는 것이 좋습니다.

1. Android Studio의 Device Manager에서 `Medium Phone`을 시작하고 기기 ID를 확인합니다.
   ...
   아래 예시의 `emulator-5556`은 실제 출력에 표시된 ID로 바꿉니다. `Small Tablet`이 아니라
   `Medium Phone` 행의 ID를 사용해야 합니다.

2. 첫 번째 터미널에서 API 및 WebRTC 시그널링 서버를 실행합니다.
   ...
   브라우저에서 `http://localhost:8001/docs`가 열리거나 다음 명령이 응답하면 준비된 상태입니다.

   ```bash
   curl http://localhost:8001/detection
   ```

3. 두 번째 터미널에서 요구조자 앱을 `Medium Phone`에 실행합니다. Android 에뮬레이터에서
   개발 PC의 localhost는 `10.0.2.2`이므로 이 주소를 사용합니다.
   ...
   처음 실행할 때 마이크 권한을 허용합니다. 로고와 `DROWNING` 로딩 화면이 사라진 뒤
   `구조 요청이 접수되었습니다` 대기 화면이 보이면 서버 연결을 기다리는 상태입니다.

4. 세 번째 터미널에서 구조자용 관제 서비스를 웹 서버로 실행합니다.
   ...
   일반 브라우저에서 `http://localhost:8080`을 직접 엽니다. 브라우저 확장 프로그램은 필요하지
   않습니다. 마이크 권한 요청이 나타나면 `허용`을 선택합니다. 웹의 마이크 입력은 보안 컨텍스트가
   필요하므로 로컬 테스트에서는 반드시 `localhost`로 접속하고, 다른 PC에서 접속할 때는 HTTPS를
   구성해야 합니다.

5. 테스트용 탐지 이벤트를 한 번 생성합니다.

   ```bash
   curl -X POST http://localhost:8001/detection \
     -H 'Content-Type: application/json' \
     -d '{"drone_id":1,"cell_id":"A0","rss_dbm":-55.0}'
   ```

   관제 화면의 탐지 패널에 `전화 연결` 버튼이 나타납니다. 요구조자 앱은 관제에서 실제로
   `전화 연결`을 누를 때 수신하고 연결을 시작하므로, 탐지만 생성한 채 오래 기다려도 통화 연결
   타임아웃이 먼저 발생하지 않습니다.

6. 관제 화면에서 `전화 연결`을 누릅니다. 양쪽 화면이 `연결 중`을 거쳐 `통화 중`으로 바뀌는지
   확인합니다. 별도 버튼을 누르지 않아도 양쪽 마이크가 열립니다.

7. 실제 음성 송수신은 다음 순서로 각각 확인합니다.

   - 관제 PC에서 말하고, `Medium Phone`의 에뮬레이터 스피커로 소리가 나는지 확인합니다.
   - 에뮬레이터는 기본적으로 Mac의 마이크 입력을 사용합니다. Mac 마이크에 말하고 관제 PC의
     이어폰으로 소리가 나는지 확인합니다.
   - 관제 화면의 마이크 입력 표시가 말할 때 반응하는지 확인합니다. 이 표시는 로컬 마이크에
     음성이 들어온다는 뜻이며, 최종 전달 여부는 반드시 상대 단말에서 들리는 소리로 확인합니다.
   - 요구조자 앱에서 마이크를 음소거했을 때 요구조자의 음성만 전달되지 않고, 관제 앱에서 오는
     음성은 계속 들리는지 확인합니다. 음소거를 해제하면 다시 양방향으로 들려야 합니다.

   소리가 들리지 않으면 브라우저 주소창의 사이트 권한에서 마이크가 허용됐는지, macOS
   `시스템 설정 > 개인정보 보호 및 보안 > 마이크`에서 브라우저가 허용됐는지, 브라우저 탭이나
   운영체제 출력이 음소거되지 않았는지 확인합니다. 에뮬레이터의 확장 설정에서 Microphone 입력이
   비활성화되지 않았는지도 확인합니다.

8. 관제 화면에서 `통화 종료`를 누릅니다. 관제는 통화 전 상태로, 요구조자 앱은 구조 요청 대기
   상태로 돌아가야 합니다. 반대로 브라우저 탭을 닫거나 네트워크를 잠시 끊었을 때는 상대 화면에
   연결 끊김이 표시되고 자동 재연결 후 수동 재시도 상태로 전환되는지도 확인할 수 있습니다.

테스트를 마치면 각 `flutter run` 터미널에서 `q`를 눌러 앱을 종료하고 서버 터미널에서
`Ctrl+C`를 누릅니다. 브라우저의 마이크 사용 표시도 관제용 Flutter 웹 프로세스를 종료하면
사라집니다.
