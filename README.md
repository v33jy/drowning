...
- 관제 앱과 요구조자 앱 모두 기본값은 `통화하기`(마이크가 계속 열린 일반 통화)입니다.
- 통화 중 `눌러서 말하기`를 선택하면 해당 단말은 버튼을 누르고 있는 동안에만 음성을 전송합니다. 상대방 음성 수신은 계속 유지됩니다.
...
6. 관제 화면에서 `전화 연결`을 누릅니다. 양쪽 화면이 `연결 중`을 거쳐 `통화 중`으로 바뀌는지
   확인합니다. 기본 `통화하기` 모드에서는 별도 버튼을 누르지 않아도 양쪽 마이크가 열립니다.
...
7. 실제 음성 송수신은 다음 순서로 각각 확인합니다.
   ...
   - 한쪽을 `눌러서 말하기`로 바꾸고, 버튼을 누르지 않았을 때는 그쪽 음성이 전달되지 않으며
     누르고 말할 때만 전달되는지 확인합니다. 반대편에서 오는 음성은 계속 들려야 합니다.
   - 양쪽의 모드를 서로 바꿔 같은 검사를 반복합니다.
